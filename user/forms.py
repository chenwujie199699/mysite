...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(
        ...
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入用户名或邮箱'}))  # required 提示不为空
    ...
    def clean(self):
        ...
        if user is None:
            ...
            if User.objects.filter(email=username_or_email).exists():
                # print("chenwujie:%s"%dir(User.objects.filter(email=username_or_email)))
...
class RegForm(forms.Form):
    username = forms.CharField(
        ...
        max_length=30,
        min_length=3,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入3-30位用户名'}))  # required 提示不为空
...
class ChangeNicknameForm(forms.Form):
    nickname_new = forms.CharField(
                                ...
                                max_length=20,
                                widget=forms.TextInput(
                                   attrs={'class': 'form-control', 'placeholder': '请输入新的昵称'}))  # required 提示不为空
